prompt *** Create Browse Table***
@CRTbrowse
prompt *** Creating Conversation Table***
@CRTconversation
prompt *** Creating Course Table***
@CRTcourse
prompt *** Creating Dorm Table***
@CRTdorm
prompt *** Creating Dance Table***
@CRTdance
prompt *** Creating FunFact Table***
@CRTfunFact
prompt *** Creating Lunch Table***
@CRTlunch
prompt *** Creating Major Table***
@CRTmajor
prompt *** Creating Mass Table***
@CRTmass
prompt *** Creating Message Table***
@CRTmessage
prompt *** Creating Minor Table***
@CRTminor
prompt *** Creating Preferences Table***
@CRTpreferences
prompt *** Creating Recommendation Table***
@CRTrecommendation
prompt *** Creating Recommender Table***
@CRTrecommender
prompt *** Creating Request Table***
@CRTrequest
prompt *** Creating State Table***
@CRTstate
prompt *** Creating Student Table***
@CRTstudent
prompt *** Creating StudentCourse Table***
@CRTstudentCourse
prompt *** Creating StudentMajor Table***
@CRTstudentMajor
prompt *** Creating StudentMinor Table***
@CRTstudentMinor

exit;
